...
class T1Symmetry:
    def __init__(self, env):
        ...
        self.num_priv = env.num_privileged_obs # Should be 14
    ...
    def mirror_obs(self, obs):
        ...
        if obs.dim() == 3:
            batch_size, num_stack, obs_dim = obs.shape
            obs_flat = obs.reshape(batch_size * num_stack, obs_dim)
            mirrored_flat = torch.matmul(self.obs_transform, obs_flat.unsqueeze(-1)).squeeze(-1)
            return mirrored_flat.reshape(batch_size, num_stack, obs_dim)
        
        else:
            # Regular observations: Shape (Batch, 74)
